...
export class SlaEscalationService {
  ...
  async runSlaCheck(now: Date = new Date()) {
    ...
    if (activeBreachedTickets.length === 0 && unbreachedTickets.length === 0) {
      return { newBreaches: 0, evaluatedEscalations: 0 };
    }
    ...
    return {
      newBreaches,
      evaluatedEscalations: activeBreachedTickets.length,
    };
  }
